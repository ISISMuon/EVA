...
def matdef2(mat, rho, thickness):
    ...
    beamwindow = Layer(
        {
            "H": {"stoich": 8, "E_d": 10, "lattice": 3, "surface": 2},
            "C": {"stoich": 10, "E_d": 28.0, "lattice": 3.0, "surface": 7.41},
            "O": {"stoich": 4, "E_d": 28.0, "lattice": 3.0, "surface": 2.0},
        },
        density=1.4,
        width=LayerThickness[0] * 1e7,
        phase=0,
    )
    ...
    air = Layer(
        {
            "C": {"stoich": 1.24e-2, "E_d": 28.0, "lattice": 3.0, "surface": 7.41},
            "O": {"stoich": 23.1781, "E_d": 28.0, "lattice": 3.0, "surface": 2.0},
            "N": {"stoich": 75.5268, "E_d": 28.0, "lattice": 3.0, "surface": 2.0},
            "Ar": {"stoich": 1.2827, "E_d": 5.0, "lattice": 1.0, "surface": 2.0},
        },
        density=1500 * 1.20479e-3,
        width=LayerThickness[1] * 1e7,
    ...
    Al_sample_holder = Layer.from_formula(
        "Al", density=2.7, width=LayerThickness[2] * 1e7, phase=0
    )  # 0.016 mm = 160000Athick sample holder

    # sample.append(Al_sample_holder)

    matlayer1 = Layer.from_formula(
        mat, density=rho, width=LayerThickness[3] * 1e7, phase=0
    )
...
def RunTRIM(target, muon_ion, number_muon, SRIMdirectory, output_directory):
    ...
    trimdataoutput = trimsim.run(
        SRIMdirectory
    )  # Simulation run by executing SRIM.exe in directory

    TRIM.copy_output_files(
        SRIMdirectory, output_directory
    )  # ouput files from SRIM copied to desired output directory
...
def CalcProfileWithMomBite(
    sample, Mom, NFinal, Mombite, SRIMdirectory, output_directory
):
    ...
    for i in range(sigmastep + 1):
        ...
        x1, y1, e1, Temp_WS = CalcProfileWithMonoMom(
            sample, P, NE, SRIMdirectory, output_directory, WriteWKSP
        )
...
def CalcProfileWithMonoMom(
    sample, Mom, NFinal, SRIMdirectory, output_directory, SaveWKSP
):
    ...
    Title_WS = (
        "TRIM Calculation for a mono momentum of "
        + str(Mom)
        + " into "
        + mat
        + " Counts = "
...
def ScanMom(
    ...
    for MomScan in my_range(MomMin, MomMax, MomStep):
        if Simtype == "Both" or Simtype == "Mono":
            writeWKSP = True
            del1, del2, del3, Name_WS = CalcProfileWithMonoMom(
                sample, MomScan, NFinal, SRIMdirectory, output_directory, writeWKSP
            )
        ...
        if Simtype == "Both" or Simtype == "Momentum Spread":
            Name_WS = CalcProfileWithMomBite(
                sample, MomScan, NFinal, Mombite, SRIMdirectory, output_directory
            )
            # Plot_Results(Name_WS, LayerThickness, LayerLabels)
            # Plot_Results(Name_WS, LayerThickness, LayerLabels)
            # Name_WS_Com = CreateLayerWKSP(Name_WS, LayerThickness, LayerLabels)
...
def SettingUp_TRIM():
    ...
    if Simtype == "Both" or Simtype == "Mono":
        WriteWKSP = True
        del1, del2, del3, Name_WS = CalcProfileWithMonoMom(
            sample, Mom, NFinal, SRIMdirectory, output_directory, WriteWKSP
        )
        # Plot_Results(Name_WS, LayerThickness, LayerLabels)
        # Name_WS_Com = CreateLayerWKSP(Name_WS, LayerThickness, LayerLabels)
        # Plot_Results_Com(Name_WS_Com, LayerThickness, LayerLabels)
