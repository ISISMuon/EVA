...
class FitTablePlotModel(QObject):
    ...
    def plot_fit_table_data(
        ...
        self.fig, self.axs = plt.subplots(1)
        filtered_data = self.filter_data_by_param(
            momentum_range, energy_range, plot_parameter
        )
        ...
        self.momentum_list, self.run_num_list, self.parameter_list, self.stderr_list = (
            map(list, zip(*combined))
        )

        for run_num, x, y, err in zip(
            self.run_num_list, self.momentum_list, self.parameter_list, self.stderr_list
        ):
            self.axs.errorbar(
                x,
                y,
        ...
        self.axs.set_ylabel(
            plot_parameter.capitalize()
            + f" ({parameter_units.get(plot_parameter, '')})"
        )
    ...
    def save_plot_data(self, file_path: str, file_extension: str, plot_parameter: str):
        if file_extension == "Text Files (*.txt)":
            with open(file_path, "w") as file:
                ...
                for run_num, momentum, param_value, stderr in zip(
                    self.run_num_list,
                    self.momentum_list,
                    self.parameter_list,
                    self.stderr_list,
