...
class FitTablePlotView(BaseView, Ui_fit_table_plot):
    ...
    def load_fit_table_file(
        self, default_dir: str = "", file_filter: str = "JSON files (*.json)"
    ) -> str:
        """Open a file dialog to select a fit table file."""
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Fit Table File", default_dir, file_filter
        )
        if path:
    ...
    def update_table(self, model):
        ...
        self.fit_table_data_table.setHorizontalHeaderItem(
            2, QTableWidgetItem(model.plot_parameter.capitalize())
        )
        for run_num, momentum, parameter, stderr in zip(
            model.run_num_list,
            model.momentum_list,
            ...
            self.fit_table_data_table.setItem(
                row_position, 0, QTableWidgetItem(str(run_num))
            )
            self.fit_table_data_table.setItem(
                row_position, 1, QTableWidgetItem(f"{momentum:.2f}")
            )
            self.fit_table_data_table.setItem(
                row_position, 2, QTableWidgetItem(f"{parameter:.4f}")
            )
            self.fit_table_data_table.setItem(
                row_position, 3, QTableWidgetItem(f"{stderr:.4f}")
            )
