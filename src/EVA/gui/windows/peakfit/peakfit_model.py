...
class PeakFitModel(QObject):
		...
		def plot_fit(self, overwrite_old: bool=True):
				...
				self.main_axs = self.axs[0] #if isinstance(self.axs, np.ndarray) else self.axs
				self.main_axs.plot(x_data, y_data, label="Best fit")
				...
				if len(x_data) == len(self.fit_result.residual):
						...
						self.residual_axs = self.axs[1]  # if isinstance(self.axs, np.ndarray) else self.axs
				...
				self.main_axs.set_ylim(self.calculate_y_range(y_data))
