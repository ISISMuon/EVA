...
class PeakFitPresenter(object):
    def __init__(self, view, model, mf_model, parent=None):
        ...
        self.view.cancel_add_peak_button.clicked.connect(
            lambda: self.set_add_peak_mode(False)
        )
        ...
        self.view.save_param_to_fit_table_button.clicked.connect(
            self.save_param_to_fit_table
        )
        ...
        self.view.save_model_fit_report_button.clicked.connect(
            self.save_model_fit_report
        )
        self.view.save_model_fitted_model_button.clicked.connect(
            self.save_model_fitted_model
        )
        self.view.save_model_plot_points_button.clicked.connect(
            self.save_model_plot_points
        )
        ...
        self.view.initial_model_params_table.contents_updated_s.connect(
            lambda: self.view.setup_modelfit_table_options(
                3, self.mf_model.initial_model_params
            )
        )

        self.view.initial_peak_params_table.contents_updated_s.connect(
            lambda: self.view.setup_peakfit_table_options(
                4, self.model.initial_peak_params
            )
        )

        # modify parameter when user edits a cell in these tables
        self.view.initial_peak_params_table.user_edited_cell_s.connect(
            self.modify_peak_parameter
        )
        self.view.initial_model_params_table.user_edited_cell_s.connect(
            self.modify_model_parameter
        )

        self.view.initial_bg_params_table.user_edited_cell_s.connect(
            lambda r, c: self.modify_bg_parameter(
                r, c, self.view.initial_bg_params_table, self.model.initial_bg_params
            )
        )
        ...
        self.view.initial_model_bg_params_table.user_edited_cell_s.connect(
            lambda r, c: self.modify_bg_parameter(
                r,
                c,
                self.view.initial_model_bg_params_table,
                self.mf_model.initial_bg_params,
            )
        )

        self.view.e_range_min_line_edit.textEdited.connect(
            lambda x: self.view.auto_e_range_checkbox.setChecked(False)
        )
        self.view.e_range_max_line_edit.textEdited.connect(
            lambda x: self.view.auto_e_range_checkbox.setChecked(False)
        )
        ...
        self.view.plot_comp_checkbox.stateChanged.connect(
            lambda check_state: self.plot_component(check_state)
        )
    ...
    def plot_initial(self):
        if len(self.model.initial_peak_params) < 1:
            self.view.display_error_message(
                message="Please add at least one peak to fit."
            )
    ...
    def start_peakfit(self):
        ...
        if self.view.auto_e_range_checkbox.isChecked():
            ...
            try:
                ...
                self.view.display_error_message(
                    message="Please specify a valid energy range."
                )
        ...
        try:
            ...
            if e.args[0] == "Not enough points":
                self.view.display_error_message(
                    message="Selected fitting range too narrow to fit curve (Not enough data points in range).\n"
                    "Either specify a wider energy range or increase initial peak width of peaks (or increase binning)."
                )

                logger.error(
                    "Not enough points to fit - aborting peakfit. \nError message: %s",
                    e.args[0],
                )
                raise e
            else:
                self.view.display_error_message(
                    message=f"An unexpected error occurred. Please ensure your initial parameters are good enough, and"
                    f" that any constraints / bounds are valid, if specified.\n"
                    f"Error message from lmfit: {e.args[0]}"
                )
                # raise e
                logger.error(
                    "Unexpected error - aborting peakfit. \nError message from lmfit: %s",
                    e.args[0],
                )
                return
            ...
            self.view.display_error_message(
                message="A recursion error occurred. If parameter constraints have been set, "
                "ensure they are not recursive."
            )
            logger.error(
                "Recusion error, likely due to recursive constraints applied - aborting peakfit."
            )
            return

        except (ValueError, IndexError) as e:
            self.view.display_error_message(
                message=f"An unexpected error occurred. Please ensure your initial parameters "
        ...
        self.view.fitted_bg_params_table.update_contents(
            self.format_params(self.model.fitted_bg_params)
        )
        ...
        if not self.model.fit_result.errorbars:
            self.view.display_error_message(
                message="Could not estimate fit errors. Please adjust your initial parameters "
    ...
    def start_model_fit(self):
        ...
        if self.view.constrain_proportions_checkbox.isChecked():
            try:
                self.mf_model.proportions_constraint = float(
                    self.view.constrain_proportions_lineedit.text()
                )
            except (ValueError, AttributeError):
                self.view.display_error_message(
                    message="Invalid proportions constraint"
                )
        ...
        try:
            ...
            self.view.fitted_model_params_table.update_contents(
                self.format_params(self.mf_model.fitted_model_params)
            )
            self.view.fitted_model_bg_params_table.update_contents(
                self.format_params(self.mf_model.fitted_bg_params)
            )

            self.view.model_params_tabs.setCurrentIndex(1)
            self.view.model_bg_params_tabs.setCurrentIndex(1)
            self.view.model_fit_report_text_browser.setText(
                self.mf_model.fit_result.fit_report()
            )

        except (TypeError, ValueError, Exception) as e:
            self.view.display_error_message(
                message=f"Unexpected error occurred: {e.args}"
            )
        ...
        for param_id, param in params.items():
            ...
            for var_name, var in param.items():
                if var.get("stderr", None) is not None:
                    ...
                    if (
                        len(item) > 18
                    ):  # if value becomes too large to fit cell, use scientific notation
    ...
    def save_params(self):
        ...
        path = self.view.get_save_file_path(
            default_dir=def_dir,
            file_filter="Peakfit parameter files (*prm)",
            default_extension=".prm",
        )
        if path:
    ...
    def save_fitted_model(self):
        ...
        path = self.view.get_save_file_path(
            default_dir=def_dir,
            file_filter="Gaussian Fit Model file (*.gfm)",
            default_extension=".gfm",
        )
        if path:
    ...
    def save_model_fitted_model(self):
        ...
        path = self.view.get_save_file_path(
            default_dir=def_dir,
            file_filter="Model Fit Model file (*.mfm)",
            default_extension=".mfm",
        )
        if path:
    ...
    def save_param_to_fit_table(self):
        ...
        if not self.model.fit_result:
            self.view.display_error_message(
                message="Please perform a peak fit to save fitted model."
            )
            return
        ...
        try:
            ...
            self.view.display_error_message(
                message="Unable to save data to fit table, ensure it is not open elsewhere."
            )
    ...
    def add_model(self):
        ...
        if path:
            ...
            self.view.initial_model_params_table.update_contents(
                self.format_params(self.mf_model.initial_model_params)
            )
