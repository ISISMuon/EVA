...
class RunNexus(Run):
    ...
    def __init__(self, raw, loaded_detectors, run_num, plot_mode, prompt_limit, comment_data, momentum):
        ...
        self.data = {key: SpectrumNexus(detector=nexus_obj.detector,run_number=nexus_obj.run_number) for key, nexus_obj in self._raw.items()}
    ...
    def _set_mode(self, plot_mode: str | None = None, prompt_limit: str | None = None):
        ...
        for detector, spectrum in self._raw.items():
            if plot_mode == "IBEX Prompt Spectrum":
                ...
                spectrum.x, spectrum.y = rebin.nxs_rebin(filtered_time_data, bin_num=100, bin_range=(0, 2000))
    ...
    def _bin_method_from_plotmode(self, plot_mode: str) -> str:
        if plot_mode in ["IBEX Prompt Spectrum", "IBEX Delayed Spectrum", "Time Plot"]:
            return "prebinned"
        elif plot_mode in ["Manual Prompt Spectrum", "Manual Delayed Spectrum", "Efficiency Spectrum"]:
            return "raw"
    ...
    def read_comment_data(self):
        ...
        return comment, start, end, f"Prompt events: {prompt_events} Delayed events: {delayed_events}"

    @classmethod
    def empty(cls) -> "Run":
        """Create an empty Run with only run_num populated."""
        return cls(run_num=0,
                   momentum=0,
                    raw={},
                    loaded_detectors=[],
