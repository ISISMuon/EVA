    ...
    def _set_mode(self, plot_mode: str | None = None, prompt_limit: int | None = None):
        ...
        for detector, spectrum in self._raw.items():
            ...
            if plot_mode == "Prompt Spectrum":
                ...
                spectrum.x, spectrum.y = rebin.nxs_rebin(prompt_spectrum_data, bin_size=bin_size_test,range=range_250)
                print(plot_mode, range_250)
                ...
                spectrum.x, spectrum.y = rebin.nxs_rebin(delayed_spectrum_data, bin_size=bin_size_test,range=range_250)
                print(plot_mode, range_250)
                ...
                spectrum.x, spectrum.y = rebin.nxs_rebin(filtered_time_data, bin_size=100, range=range_250)
                print(plot_mode, 100)

                self.data[detector].x = spectrum.x
                self.data[detector].y = spectrum.y

            elif plot_mode == "2D Time-Energy Spectrum":
                spectrum.x = self._raw[detector].count
                spectrum.y = self._raw[detector].t
