...
def load_gamma_data():
    ...
    for i in range(minZ, maxZ):
        Z = i + 1
        if len(str(Z)) == 1:
            data, elem_name = decode_gammas(
                os.path.abspath(os.path.join(gamma_levels_path, f"z00{str(Z)}.dat"))
            )
        if len(str(Z)) == 2:
            data, elem_name = decode_gammas(
                os.path.abspath(os.path.join(gamma_levels_path, f"z0{str(Z)}.dat"))
            )
        if len(str(Z)) == 3:
            data, elem_name = decode_gammas(
                os.path.abspath(os.path.join(gamma_levels_path, f"z{str(Z)}.dat"))
            )
...
def decode_gammas(filename):
    f = open(filename, "r")
    record = namedtuple("Isotope", "SYMB A Z Nol Nog Nmax Nc Sn Sp")
    columns1 = (
        (0, 5),
        (6, 10),
        (11, 15),
    ...
    record2 = namedtuple("Level", "N1 Elv s p T_half Ng J")
    columns2 = ((0, 3), (5, 14), (16, 20), (21, 23), (25, 33), (34, 38), (39, 39))

    record3 = namedtuple("Gammas", "Nf Eg Pg Pe ICC")
