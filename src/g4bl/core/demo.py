...
def case2(vis_mode, beam_off, shift = 0, plot_individual = False): 
    ...
    Slab(name="Copp", material="Cu", color="1,1,0", thickness=0.5, num_samples=num4),
    Slab(name="Alum2", material="Al", color="0,0,1", thickness=0.05, num_samples=num1),
        ]
    model.stack_layer_boundaries(shapes)
    main(model,shapes=shapes, vis_mode=vis_mode, beam_off=beam_off, plot_individual=plot_individual)

def case3(vis_mode, beam_off, plot_individual = False): 
    model = G4blModel()
    shapes = [
    Sphere("s1", "G4_Si", (0, -2, 2), "1,0,0", radius=(0, 1.5), num_samples=50),
    # Cylinder("c1", "G4_Al", (0, 0, 4), "0,1,0", radius=(0, .5), length=2, num_samples=20),
    Sphere("s2", "Pb", (0, 2, 2), "0,0,1", radius=(0, 1.5), num_samples=40)
    ]
    main(model, shapes=shapes, vis_mode=vis_mode, beam_off=beam_off, plot_individual=plot_individual)
...
case2(vis_mode=False, beam_off=False)
