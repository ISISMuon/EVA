...
class TRIMInput(object):
    ...
    def _write_elements(self):
        elements_str = ("Target Elements:    Z   Mass [amu]") + self.newline
        index = 1
        for layer in self._trim.target.layers:
            for element in layer.elements:
                elements_str += (
                    "Atom {} = {} =     {} {}".format(
                        index, element.symbol, element.atomic_number, element.mass
                    )
                    + self.newline
                )
                index += 1
    ...
    def _write_layer(self):
        ...
        for layer_index, layer in enumerate(self._trim.target.layers, start=1):
            ...
            layer_str += " 0.0" * (
                self.srim_num_elements - element_index - len(layer.elements)
            )
            element_index += len(layer.elements)
            layers_str.append(layer_str)
        return (
            self.newline.join([layers_str_header_1, layers_str_header_2] + layers_str)
            + self.newline
        )
    ...
    def _write_solid_gas(self):
        return (
            ...
            + self.newline
            + " ".join(str(layer.phase) for layer in self._trim.target.layers)
            + self.newline
        )
    ...
    def _write_displacement_energies(self):
        ed_str = ("Individual target atom displacement energies (eV)") + self.newline
    ...
    def _write_version(self):
        return (
            ...
            + self.newline
            + "{}".format(self._trim.settings.version)
            + self.newline
        )
...
class SRInput(object):
    ...
    def _write_filename(self):
        return (
            ...
            + ("---Output File Name")
            + self.newline
            + "{}".format(self._sr.settings.output_filename)
            + self.newline
        )
    ...
    def _write_ion(self):
        return (
            ("---Ion(Z), Ion Mass(u)")
            + self.newline
            + "{} {}".format(self._sr.ion.atomic_number, self._sr.ion.mass)
            + self.newline
        )
    ...
    def _write_layer_info(self):
        return (
            ...
            + self.newline
            + "{}".format(len(self._sr.layer.elements))
            + self.newline
        )
    ...
    def _write_output_options(self):
        return (
            ...
            + self.newline
            + "{}".format(self._sr.settings.output_type)
            + self.newline
        )
    ...
    def _write_ion_energy_range(self):
        return (
            ...
            + self.newline
            + "{} {}".format(
                self._sr.settings.energy_min / 1.0e3, self._sr.ion.energy / 1.0e3
            )
            + self.newline
        )
