...
default_peaks_list_mudirac = [[1050, 1210], [1080]]

default_sigma_list_mudirac = [10, 5]

target_result_list_mudirac = [
    {
        "Aa": [1050, 1040, 1060, 1030, 1070, 1020, 1080],
        "Ab": [1210, 1200, 1220, 1190, 1230, 1180, 1240],
    },
    {"Aa": [1080, 1070]},
]
...
target_result_list_legacy = [
    {
        "Aa": [1342.0, 1340.0, 1346.0, 1347.0],
        "Ab": [1341.0, 1741.0, 1740.0, 1742.3, 1747.0],
    },
    {"Aa": [1080, 1070]},
...
default_peaks_list_gamma = [[20.500]]
...
class TestDatabasesGetMatch:
    ...
    @pytest.mark.parametrize(
        ...
        list(
            zip(
                default_peaks_list_legacy,
                default_sigma_list_legacy,
                target_result_list_legacy,
            )
        ),
    )
    def test_muon_legacy_db_match(
        self, qapp, default_peaks, default_sigma, target_result
    ):
        # Switch to legacy db
        app = get_app()
        ...
        app.reset()
        assert matches.keys() == target_result.keys(), (
            "Data in legacy muon database did not match expected"
        )
        assert all(
            [
                [energy in target_result[elem] for energy in matches[elem]]
                for elem in matches
            ]
        ), "Data in legacy muon database did not match expected"
    ...
    @pytest.mark.parametrize(
        ...
        list(
            zip(
                ...
                default_sigma_list_mudirac,
                target_result_list_mudirac,
            )
        ),
    )
    def test_muon_mudirac_db_match(
        self, qapp, default_peaks, default_sigma, target_result
    ):
