...
class TestPlotWindow:
    ...
    def setup(self, qtbot):
        ...
        self.presenter = self.window._presenter
    ...
    def test_find_peaks_plotting_on_button_click(self, qtbot):
        ...
        init_data_ax0 = list(self.view.plot.canvas.axs[0].collections)
        init_data_ax1 = list(self.view.plot.canvas.axs[1].collections)

        # click on peak fitting button - ONLY TESTS SCIPY FIND PEAKS METHOD
        qtbot.mouseClick(self.view.find_peaks_button, Qt.MouseButton.LeftButton)

        # data_ax0 = self.window.plot.canvas.axs[0].collections[1].get_offsets().data - HOW TO GET THE POINT POSITIONS
        data_ax0 = list(self.view.plot.canvas.axs[0].collections)
